...
void Light::drawAlpha(World *world) {
	glBegin(GL_TRIANGLE_FAN);
...
void Light::drawShadows(World *world) {
	for (int i = 0; i < world->polygons.size(); i++) {
		std::vector<m_Edge*> away = world->polygons[i]->getNotFacingEdges(pos);
